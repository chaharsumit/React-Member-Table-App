...
import {
  filterCompany,
  removeFromCompanyFilter,
  removeFromStatusFilter,
  filterStatus,
  toggleCompany,
  toggleStatus
} from "../store/action";
...
function Filter(props) {
  ...
  return (
    <div className="flex col-gap-2 mt-2 mb-2">
      <button
        className="btn btn-filter bg-light flex align-center"
        onClick={toggleCompanyDropdown}
      >
        Company({companyFilter.length}) ˅
      </button>
      {members.length && companyDropdownIsOpen ? (
        <ul className="company-dropdown pos-absolute">
        
          <CompanyDropdown
            members={members}
            companyFilterArr={companyFilter}
            filter={handleCompanyFilter}
          />
        </ul>
        ) : (
          ""
      ...
      {members.length && statusDropdownIsOpen ? (
        <ul className="status-dropdown pos-absolute">
          <StatusDropdown
            members={members}
  ...
  return (
    <>
      <li
        id="selectAll"
        key="selectAll"
        onClick={event => props.filter(event, companies)}
      >
        <input
          type="checkbox"
          checked={
            props.companyFilterArr.length === companies.length ? true : false
          }
        />
        Select All
      </li>
      {companies.map(company => (
        <li
          ...
          onClick={event => props.filter(event, companies)}
        >
          {props.companyFilterArr.includes(company) ? (
  ...
  return status.map(status => (
    <li key={status} id={status} onClick={event => props.filter(event)}>
      <input
        type="checkbox"
        id={status}
        checked={props.statusFilter === status ? true : false}
      />
      {status}
    </li>
  ));
